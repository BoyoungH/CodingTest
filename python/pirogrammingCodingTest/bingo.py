...
class Game:
    ...
    def start_game(self):
        ...
        self.my_player = input('당신의 이름을 입력해주세요. ')
        ...
        for player in self.players:
            while True:
                player.table = [[0, 0, 0], [0, 0, 0], [0, 0, 0]]
                selected_numbers = random.sample(self.deck, k=9)
                for i in range(3):
                    for j in range(3):
                        player.table[i][j] = selected_numbers.pop()
    ...
    def set_next_player(self, round_num):
        ...
        next_player = self.players[(round_num-1) % len(self.players)]
    ...
    def count_bingo(self):
        ...
        for player in self.players:
            ...
            for i in range(3):
                if player.table[0][i] == player.table[1][i] == player.table[2][i] == 0:
                    player.bingo_cnt += 1               
            ##### END OF TODO 3-(2)(문제와 본 라인 사이에 코드를 작성하세요.) #####

            # TODO 3-(3): 플레이어의 빙고판 대각선(좌상단 -> 우하단) 확인
            if player.table[0][0] == player.table[1][1] == player.table[2][2] == 0:
                player.bingo_cnt += 1
            ##### END OF TODO 3-(3)(문제와 본 라인 사이에 코드를 작성하세요.) #####

            # TODO 3-(4): 플레이어의 빙고판 대각선(우상단 -> 좌하단) 확인
            if player.table[0][2] == player.table[1][1] == player.table[2][0] == 0:
                player.bingo_cnt += 1
    ...
    def game_result(self):
        ...
        self.players.sort(key=lambda x: (-x.bingo_cnt, x.name))
        ##### END OF TODO 5-(1)(문제와 본 라인 사이에 코드를 작성하세요.) #####

        # TODO 5-(2): 사용자의 경우 이름 옆에 *을 붙여서 출력해주세요.
        # 점수가 같으면 등수도 같도록 반드시 동점자 처리를 해주세요.
        rank = 0  # 초기 등수
        for i in range(len(self.players)):
            if i > 0 and self.players[i].bingo_cnt == self.players[i - 1].bingo_cnt:
                if self.players[i].name == self.my_player:
                    print(f'{rank}등 - *{self.players[i].name}* : 빙고 {self.players[i].bingo_cnt}번')
                else:
                    # 이전 플레이어와 빙고 수가 같으면 동점자 처리
                    print(f'{rank}등 - {self.players[i].name} : 빙고 {self.players[i].bingo_cnt}번')
            else:
                # 이전 플레이어와 빙고 수가 다르면 새로운 등수 부여
                rank += 1
                if self.players[i].name == self.my_player:
                    print(f'{rank}등 - *{self.players[i].name}* : 빙고 {self.players[i].bingo_cnt}번')
                else:
